...
class core_ai_backend implements ai_backend {
    ...
    /**
     * Perform a text extraction request via core AI subsystem.
     *
     * Note: The encoded data parameter is ignored since core AI only supports
     * text prompts. This method is provided for interface compatibility but
     * will produce poor results for image-based extraction.
     *
     * @param string $prompt The prompt text.
     * @param string $encodeddata Base64-encoded data URL (ignored by core AI).
     * @param int $contextid The context ID for the AI request.
     * @return string The AI response text.
     * @throws \moodle_exception If the request fails or core AI is not available.
     */
    public function extract_text_from_encoded_data(string $prompt, string $encodeddata, int $contextid): string {
        ...
        if (
            ...
            throw new \moodle_exception('error_airequestfailed', 'local_ai_content');
        }
