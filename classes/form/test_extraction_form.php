 ...
 */
class test_extraction_form extends \moodleform {
    /**
    ...
    protected function definition() {
        ...
        $mform->addElement(
            'filepicker',
            'testfile',
            get_string('test_extraction_file', 'local_ai_content'),
            null,
            ['maxbytes' => 10 * 1024 * 1024, 'accepted_types' => '*']
        );
        $mform->addRule('testfile', get_string('required'), 'required', null, 'client');

        $this->add_action_buttons(false, get_string('test_extraction_submit', 'local_ai_content'));
    }
}
